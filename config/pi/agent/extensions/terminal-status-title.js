// Live agent status in the terminal title (idle / working spinner / done).
// Adapted from Kun Chen's published Pi config:
// https://github.com/kunchenguid/dotfiles/blob/main/home/.pi/agent/extensions/terminal-status-title.js
// Herdr's agent sidebar shows terminal_title_stripped, so this is visible there too.
...
function formatTitle(pi, ctx, status, spinnerFrame) {
  ...
  const suffix =
    rawTitle === DEFAULT_TITLE ? DEFAULT_TITLE : `${PREFIX} | ${truncateTitle(rawTitle)}`;
